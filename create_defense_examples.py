...
jailbreak_method_list = ['DirectRequest', 'HumanJailbreaks', 'AutoDAN', 'PAP', 'GCG', 'AutoPrompt', 'PEZ', 'GBDA', 'UAT']
...
# defender_list = ['ppl', 'retokenization', 'self-reminder', 'llama_guard_3_1B']
defender_dict = {
    "ppl": "PPL",
...
defender_output_name_map = {"ppl": "ppl_score", "retokenization": "retokenized_test_case",
                                'self-reminder': "modified_user_prompt", "llama_guard_3_1B": "defender_output"}


# 每个slm family选择一个slm
slm_family_to_slm = {
    "LLaMA 3.2 family": "llama3_2_1b_instruct",
    "DeepSeek-R1 family": "DeepSeek-R1-Distill-Qwen-1.5B",
...
parser.add_argument('--output_file', type=str, default='/data2/zwh/demos/SLM_Jailbreak_website/defense_examples.json', help='Path to save the results summary.')
...
if __name__ == '__main__':
    ...
    # 遍历 jailbreak_method_list 和 model_list
    for family, model in slm_family_to_slm.items():
        ...
        for method in jailbreak_method_list:
            print(f"Processing method {method}")
            if method not in results_summary[family]:
                results_summary[family][method] = {}

            for defender, defender_title in defender_dict.items():
                if defender not in results_summary[family][method]:
                    results_summary[family][method][defender_title] = {}

                if method == 'DirectRequest':
                    checked_result_path = os.path.join(base_path, method, 'default', 'defense_results', defender, f'{model}.json')
                elif method == 'HumanJailbreaks':
                    checked_result_path = os.path.join(base_path, method, 'random_subset_5', 'defense_results', defender, f'{model}.json')
                elif method == 'PAP':
                    checked_result_path = os.path.join(base_path, method, 'top_5', 'defense_results', defender, f'{model}.json')
                else:
                    checked_result_path = os.path.join(base_path, method, model, 'defense_results', defender, f'{model}.json')
                ...
                for category, ids in category_to_ids.items():
                    ...
                    results_summary[family][method][defender_title][category] = [
                        question, 
                        result_data[selected_id][0]["test_case"], 
                        result_data[selected_id][0][defender_output_name_map[defender]],
